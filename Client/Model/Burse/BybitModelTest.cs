...
namespace Client.Model.Burse
{
    public class BybitModelTest : BurseModel
    {
        ...
        {
            _rest = new(new BybitRestApiClientOptions() 
            {
                RawResponse = true,
            });

            _socket = new(new BybitWebSocketClientOptions()
            {
                RawResponse = true,
            });
        }
        protected override async Task<bool> GetConnection()
        {
            ...
            _rest.SetApiCredentials(api, secret);
            _socket.SetApiCredentials(credentials);

            var result = false;
            var ticker = await _rest.Market.GetSpotTickersAsync("BTCUSDT");
            var balance = await _socket.SubscribeToWalletUpdatesAsync(OnAccountUpdated);
        ...
        {
            var account = await _socket.SubscribeToPositionUpdatesAsync(OnPositionUpdated);
            var orders = await _socket.SubscribeToOrderUpdatesAsync(OnOrderUpdated);
        }

        private void OnAccountUpdated(WebSocketDataEvent<BybitAccountBalance> update)
        {
            if (update != null)
            {
        ...
        private void OnPositionUpdated(WebSocketDataEvent<BybitPositionUpdate> update)
        {
            if (update != null)
            {
                lock (update)
        ...
        private void OnOrderUpdated(WebSocketDataEvent<BybitOrderUpdate> update)
        {
            if (update != null)
            {
                //foreach (var data in update.Data)
        ...
        {
            var orders = await _rest.Trade.GetOrdersAsync(BybitCategory.Linear);
            if (orders.Data != null)
            {
        ...
        protected override async Task PlaceOrder(SourceList<Order> orders, decimal limit)
        {
            if (IsConnected)
            {
                if (Balance > limit)
                {
                    List<BybitBatchPlaceOrderRequest> _ = [];
                    ...
                    foreach (var order in orders.Items)
                    {
                        BybitOrderSide side = order.Side.Equals("Sell") ? BybitOrderSide.Sell : BybitOrderSide.Buy;
                        BybitOrderType type = BybitOrderType.Limit;
                        _.Add(new()
                        {
                    ...
                    var trade = await _rest.Trade.PlaceOrdersAsync(BybitCategory.Linear, _);
                }
            }
        }
        protected override async void UpdateOrderByTime(Order order, StockInfo stock)
        {
            var _ = await _rest.Market.GetSpotTickersAsync(order.InstrumentId);
        ...
        {
            var trade = await _rest.Trade.PlaceOrderAsync(BybitCategory.Linear, "BTC-USDT", BybitOrderSide.Sell, BybitOrderType.Market, 0.00001m, clientOrderId: "VitalUd20010");
        }
    }
}
